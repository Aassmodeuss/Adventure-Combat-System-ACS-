...
globalThis.MainSettings = (class MainSettings {
    ...
    */
    ...
    //—————————————————————————————————————————————————————————————————————————————————
    /*
    Adventure Combat System (ACS)
    Thin integration layer that leverages Auto-Cards for card lifecycle. Toggle-able via MainSettings.
    */
    static ACS = {
        // Is ACS already enabled when the adventure begins?
        DEFAULT_DO_ACS: true
        // (true or false)
        ,
        // Show brief toasts when ACS performs setup
        DEFAULT_SHOW_TOASTS: true
        // (true or false)
    };
...
function AutoCards(inHook, inText, inStop) {
    ...
    */
    ...
    function hoistConst() {
        // Emulate a private slot compatible with older runtimes (no #fields support)
        ...
        return (class Const {
            ...
            declare(...args) {
                ...
                } else if (!_store.has(this)) {
                    _store.set(this, null);
                    return this;
                } else if (_store.get(this) === null) {
            ...
            static throwError(...args) {
                ...
                if (error.stack) {
                    codomain.initialize(error.stack
                        ...
                        .replace(/Error:|at\s*(?:(?:#|Const\.)?throwError|Const\.(?:declare|initialize|read)|new\s*Const)\s*\(\d+:\d+\)/gi, "")
...
// Minimal ACS scaffold that integrates with Auto-Cards
function ACS(inHook, inText, inStop) {
    "use strict";
    const HOOK = inHook;
    const TEXT = (inText ?? "");
    const STOP = (inStop ?? false);
    const API = (function() {
        try {
            if (globalThis && globalThis.AutoCardsTestAPI) return globalThis.AutoCardsTestAPI;
        } catch {}
        try { return AutoCards().API; } catch { return null; }
    })();
    /*
    ACS Inventory Tagging
    - Runs in: context (injects model instruction), output (parses tags and updates Inventory card)
    - Standing instruction asks the model to append wrapper tags at the very end of output for inventory changes:
        Add items: {\\pickup} item name xN{\\!pickup}
        Remove items: {\\!drop} item name xN{\\drop}
      Multiple tags may be included. If no inventory change, model should add no tags.
    - Output hook strips these tags from visible text and maintains quantities on the "Inventory" story card as bullet lines "- name xN".
    */
    // Read scenario defaults (with safe fallbacks)
    const A = (function(){
        try {
            const ms = new MainSettings("ACS");
            const cfg = { DO_ACS: true, SHOW_TOASTS: true };
            ms.merge({
                DEFAULT_DO_ACS: cfg.DO_ACS,
                DEFAULT_SHOW_TOASTS: cfg.SHOW_TOASTS
            });
            return {
                DO_ACS: !!MainSettings.ACS?.DEFAULT_DO_ACS,
                SHOW_TOASTS: !!MainSettings.ACS?.DEFAULT_SHOW_TOASTS
            };
        } catch { return { DO_ACS: true, SHOW_TOASTS: true }; }
    })();
    if (!API || !A.DO_ACS) {
        // Pass-through
        switch(HOOK){
        case "context": return [TEXT, STOP];
        default: return TEXT;
        }
    }
    // Reserved ACS titles (kept banned for collision avoidance)
    ...
    // Ensure bans include reserved titles (preserve existing bans)
    function ensureReservedBans(){
        try {
            const existing = API.getBannedTitles();
            const merged = [...existing];
            const lower = new Set(existing.map(s => s.toLowerCase()));
            for (const t of RESERVED_TITLES) {
                if (!lower.has(t.toLowerCase())) merged.push(t);
            }
            API.setBannedTitles(merged);
        } catch { /* noop */ }
    }
    // Helper: ensure a singleton core card exists with proper auto headers
    function ensureCoreCard(title, entryBody){
        try {
            let card = API.getCard(c => (c.title === title));
            if (!card) {
                const entry = ("{title: " + title + "}\n" + (entryBody || "Managed by ACS.")).trim();
                const description = (
                    title === "Inventory"
                ...
                if (A.SHOW_TOASTS) { state.message = (state.message || "") + "\nACS: Created '" + title + "'"; }
            }
        } catch { /* noop */ }
    }
    ...
    // Basic plural normalization on the last word; used for case-insensitive merging
    function normalizeKey(name){
        ...
        } else if (/(xes|ses|zes|ches|shes)$/.test(last)) {
    ...
    function buildInventoryStandingPrompt(){
        return (
            ...
            "- ABSOLUTE NO-MARKUP: plain prose only; never any <...>/headers. Only {\\pickup}...{\\!pickup} and {\\!drop}...{\\drop}.\n" +
            "- Self-check: if '<' or '>' appears, remove it; keep ACS wrappers only.\n" +
            "- Tag ONLY when inventory changes.\n" +
            "- Format (end-only): {\\pickup} item[cat] xN, ...{\\!pickup} and {\\!drop} item xN, ...{\\drop}.\n" +
            "- Names: singular, lowercase; keep material words (e.g., brass compass); no size adjectives. Quantities as xN. Coins: gold/silver/copper coin xN.\n" +
            "- One quantity per item. Use nouns from narration; don't invent descriptors.\n" +
            "- Quantifiers w/o number (handful/bundle/stack/pile/pouch/bag/sack): choose 1–9 (for drop, default x1). For coins choose 3–7. ALWAYS add [item] on coin entries.\n" +
            "- Multiple items: one block per action. If both occur, MUST order: pickup first, then drop. Put both blocks contiguously at the very end.\n" +
            "- Discovery on person (belt/pocket/cloak/pack/backpack/satchel): pickup x1. Container rule: if the noun is a container (coin pouch, waterskin, backpack, satchel, bag, sack, quiver, sheath, holster, scabbard), tag the container itself with [item] x1; DO NOT infer contents unless explicit counts.\n" +
            "- Never tag observations/dialogue/plans/hypotheticals/failed actions.\n" +
            "- If a number is stated, use that exact count.\n" +
            "- Category (pickup only): EVERY item must have [weapon]/[armor]/[item]. Coins MUST use [item]. If unsure or missing, use [item]. Never put categories in drop.\n" +
            "- Drop block: never include categories.\n" +
            "- MUST drop on: drop/discard/leave/place/set/put/lay.\n" +
            "- Usage is drop: shoot/fire/loose/launch/hurl/throw; drink/quaff/swallow/eat/devour/consume; burn; pay/spend; use charges → {\\!drop} item xN.\n" +
            "- Thrown weapons MUST drop; later recovery uses {\\pickup} throwing knife[item] x1{\\!pickup}.\n" +
            "- Consumption MUST drop (even if you tag healing inline); append {\\!drop}...{\\drop} at the very end.\n" +
            "- Self-check: if narration includes drink/quaff/swallow/eat/devour/consume and you produced no {\\!drop}, append a {\\!drop} for the consumed item(s) now (default x1).\n" +
            "- Count for usage: if given, use it; else x1.\n" +
            "- Drawing/readying/equipping from pack/belt/hand alone is NOT pickup/drop; no tags unless also discovered (pickup) or set/left (drop).\n" +
            "- Self-check: if you produced {\\!drop} but narration only says remove/draw/ready/equip (and no leave/drop/set/put/lay and no usage verbs), delete the drop block.\n" +
            ...
            "          {\\!drop} arrow x3{\\drop}\n" +
            "          Drink: narrative + {\\!drop} healing potion x1{\\drop}\n" +
            "          Negative: 'remove torch from pack' → no tags\n" +
            "          Both: {\\pickup} apple[item] x2{\\!pickup}{\\!drop} copper ore x1{\\drop}\n" +
            "</SYSTEM>\n"
        );
    }
    ...
    function buildInjuryStandingPrompt(){
        return (
            "\n-----\n" +
            "<SYSTEM>\n" +
            "Injury Tagging (player; inline; sentence-level):\n" +
            "- ABSOLUTE NO-MARKUP; only {\\injury}...{\\!injury} and {\\heal}...{\\!heal}.\n" +
            "- Self-check: if '<' or '>' appears, remove it; keep ACS wrappers only.\n" +
            "- When the PLAYER is injured or healed, wrap at least the entire sentence; optionally two contiguous sentences if needed (cause + immediate effect). Use exact original wording; no added/changed words; no severity/ranking.\n" +
            "- Choose the sentence whose main action causes injury (strike/slash/bite/fall/burn) or performs healing. Tag each event separately; if both occur, injury tags come before heal tags.\n" +
            "- Do NOT tag near-misses/blocks/flavor. If cues (bite/slash/wound/bleed/break/fracture/burn/heal/bandage/potion) appear and you emitted no tags, wrap the sentence.\n" +
            "Examples: {\\injury}The wolf clamps down on your forearm; sharp teeth puncture skin and pain flares.{\\!injury}\n" +
            "          {\\heal}You steady your breathing and bandage your forearm to stop the bleeding.{\\!heal}\n" +
            "          {\\injury}A stone drops from above, slamming into your ribs and knocking the wind from you.{\\!injury} {\\heal}You quickly wrap your ribs with bandages to support your breath.{\\!heal}\n" +
            "</SYSTEM>\n"
        );
    }
    // Parse inventory lines from Inventory card description into a name->qty map
    function parseInventoryMap(desc){
        const map = new Map();
        if (typeof desc !== "string" || !desc) return map;
        const lines = desc.split(/\r?\n/);
        for (const line of lines) {
            const t = line.trim();
            if (!t.startsWith("- ")) continue;
            const body = t.slice(2).trim();
            // Expect trailing xN, but accept without to default to 1
            let name = body;
            let qty = 1;
            let cat = null;
            const mQty = body.match(/\s+x(\d+)\s*$/i);
            if (mQty) {
                qty = Math.max(0, parseInt(mQty[1], 10) || 0);
            ...
            const newQty = (prev ? prev.qty : 0) + qty;
            // Persist first known category; if none stored yet and we parsed one, adopt it
            const nextCat = prev ? (prev.cat || cat) : cat;
            map.set(key, { name: (prev ? prev.name : name), qty: newQty, cat: nextCat });
        }
        return map;
    }
    // Serialize inventory map back to description while preserving AC memory header
    function writeInventoryDescription(originalDesc, invMap){
        // Try to preserve the existing AC memory header, but force updates:false for Inventory
        const DEFAULT_HEADER_TOP = "Auto-Cards will contextualize these memories:";
        let headerTop = DEFAULT_HEADER_TOP;
        let headerCfg = "{updates: false, limit: 2750}";
        if (typeof originalDesc === "string" && originalDesc) {
            const descLines = originalDesc.split(/\r?\n/);
            const i = descLines.findIndex(l => l.trim().startsWith(DEFAULT_HEADER_TOP));
            if (i !== -1) {
                headerTop = descLines[i].trim();
                const cfg = (descLines[i + 1] || "").trim();
                // Normalize to updates:false; preserve limit if present
                const m = cfg.match(/\{\s*updates\s*:\s*(true|false)\s*,\s*limit\s*:\s*(\d+)\s*\}/i);
                const limit = m ? (m[2] || "2750") : "2750";
                headerCfg = `{updates: false, limit: ${limit}}`;
            }
        }
        const lines = [];
        lines.push(headerTop);
        lines.push(headerCfg);
        // Emit items deterministically (alphabetical by name)
        const items = Array.from(invMap.values()).filter(v => 0 < v.qty);
        items.sort((a, b) => a.name.localeCompare(b.name));
        for (const it of items) {
            const cat = it.cat ? `[${String(it.cat).toLowerCase()}]` : "";
            lines.push("- " + it.name + (cat ? ("" + cat) : "") + " x" + it.qty);
        }
        return lines.join("\n");
    }
    // Build a concise inventory snapshot for context injection
    function buildInventoryContextBlock(){
        try {
            const card = API.getCard(c => (c.title === "Inventory"));
            let invMap = new Map();
            if (card && typeof card.description === "string") {
                invMap = parseInventoryMap(card.description);
            }
            const items = Array.from(invMap.values()).filter(v => 0 < v.qty).sort((a,b) => a.name.localeCompare(b.name));
            // Cap to avoid bloating context
            const MAX_ITEMS = 50; const MAX_CHARS = 1200; // conservative
            const chosen = items.slice(0, MAX_ITEMS);
            const lines = chosen.map(it => {
                const cat = it.cat ? `[${String(it.cat).toLowerCase()}]` : "";
                return "- " + it.name + (cat ? cat : "") + " x" + it.qty;
            });
            // Trim by char budget
            let acc = [];
            let total = 0;
    ...
    function logInjuryEvents(events){
        ...
        try {
            ...
            if (iTop === -1) {
                lines = [DEFAULT_HEADER_TOP, "{updates: true, limit: 2750}"]; // fresh header
            } else {
    ...
    function applyInventoryDeltas(invMap, deltas){
        for (const d of deltas) {
            ...
            if (!raw) continue;
            const key = normalizeKey(raw); // normalize to lowercase singular-ish key
            const current = invMap.get(key) || { name: key, qty: 0, cat: null };
            let qty = current.qty + (d.type === "pickup" ? d.qty : -d.qty);
            if (qty < 0) qty = 0;
            // prefer the canonical lowercase key as the stored name to match prompt guidance
            const cat = (d.type === "pickup" && d.cat) ? (current.cat || d.cat) : current.cat;
            invMap.set(key, { name: current.name || key, qty, cat });
        }
        return invMap;
    }
    ...
    function extractInventoryTags(outText){
        ...
        const patterns = [
            { type: "pickup", open: /\{\\pickup\}/g, close: /\{\\!pickup\}/g },
            { type: "drop", open: /\{\\!drop\}/g, close: /\{\\drop\}/g }
        ];
        // For robustness with multiple tags, iterate until no more matches
        for (const p of patterns) {
            let mOpen, startIdx = 0;
            while ((mOpen = p.open.exec(text)) !== null) {
                const openIdx = mOpen.index;
                // Find the next close after open
                p.close.lastIndex = openIdx + mOpen[0].length;
                const mClose = p.close.exec(text);
                if (!mClose) break; // malformed; stop on this pattern
                const content = text.slice(openIdx + mOpen[0].length, mClose.index);
                // Parse content into one or more items: "name xN, name2 xM"
                const raw = content.trim();
                if (raw) {
                    ...
                    for (const seg of items) {
                        ...
                        const qtys = [];
                        let category = null;
                        // Peel off any number of trailing tokens: quantities (xN) and categories [..]
                        // Keep removing from the end until no more matches
                        for (;;) {
                            const mQty = rest.match(/\s+x(\d+)\s*$/i);
                            if (mQty) {
                                const q = Math.max(1, parseInt(mQty[1], 10) || 1);
                                qtys.push(q);
                        ...
                        // Remove any stray xN fragments that might remain inside the name
                        rest = rest.replace(/\s+x\d+(?=\s|$)/ig, "").trim();
                        const qty = qtys.length ? Math.max.apply(null, qtys) : 1;
                        const name = rest;
                        if (name) deltas.push({ type: p.type, name, qty, cat: (p.type === "pickup" ? category : null) });
                    }
                }
                // Remove the tag pair from output
                const before = text.slice(0, openIdx);
                const after = text.slice(mClose.index + mClose[0].length);
                text = before + after;
                // Reset regex positions because text changed
                p.open.lastIndex = 0; p.close.lastIndex = 0;
            }
        }
    ...
    switch(HOOK){
    case "input": {
        // Command: /inv — create a story card for each inventory item without one
        try {
            const t = String(TEXT || "");
            if (/^\s*\/inv\b/i.test(t)) {
                ensureCoreCards();
                const inv = API.getCard(c => (c.title === "Inventory"));
                let created = [];
                if (inv && typeof inv.description === "string") {
                    const map = parseInventoryMap(inv.description);
                    // Build a set of existing titles for quick lookup
                    const existingTitles = new Set();
                    try {
                        const allCards = API.getCard(() => true, true) || [];
                        for (const c of allCards) { if (c && c.title) existingTitles.add(String(c.title)); }
                    } catch { /* fallback: rely on API.getCard lookups below */ }
                    for (const { name, qty, cat } of map.values()) {
                        if (!name) continue;
                        if (!(qty > 0)) continue; // ignore zero-quantity items
                        const title = name.replace(/\b\w/g, m => m.toUpperCase());
                        // Skip reserved titles and skip if card already exists
                        if (RESERVED_TITLES.includes(title)) continue;
                        ...
                        const detailsByType = {
                            ...
                            item: (
                                "Describe only the item's intrinsic properties: purpose/function, operating mechanism (how it is used), materials and craftsmanship, condition and durability, notable features or inscriptions, and usage constraints (e.g., requires fire, fragile, consumes charges). Example: 'A brass compass with a glass face and cardinal engraving, its needle responsive and casing snug, reliable in fair conditions but prone to fogging in wet weather.'"
                            )
                        };
                        const hardRules = (
                            "Hard rules: strictly describe the object only. No story or scene content whatsoever (no creatures, characters, dialogue, actions, or locations). Do not mention the player, ownership, or usage in the current scene. Do not output the {title: ...} header again. The first sentence must begin with the exact item title as the first words (e.g., '" + title + " ...'). No bullets or lists; write a single compact paragraph of 3–6 sentences. No directives or tips. No counts or inventory mechanics. Do not reference any inventory lists, other items, or contextual summaries; focus solely on this one item."
                        );
                        // ACS /inv item generation:
                        // Do not restate the base Auto-Cards generation prompt. We rely on Auto-Cards' default prompt
                        // and only append category-specific guidance for inventory objects below.
                        const entryPromptDetails = (
                        ...
                        try {
                            ...
                            if (ok) created.push(title);
                        } catch { /* skip on failure */ }
                    }
                }
                // If we queued any generations, suppress inventory prompts/context until those cards exist
                try {
                    if (created.length) {
                        const bag = (state._ACS && typeof state._ACS === "object") ? state._ACS : {};
                        // Merge newly created titles into a pending set we will monitor in context
                        const prior = Array.isArray(bag.pendingInvTitles) ? bag.pendingInvTitles : [];
                        const mergedSet = new Set([...prior, ...created]);
                        bag.pendingInvTitles = Array.from(mergedSet);
                        // Add a safety TTL to avoid indefinite suppression if a generation fails silently
                        const BASE_TTL = 20; // ~many turns; adjusted to cover multi-item queues
                        const prevTTL = Number(bag.pendingInvTTL || 0);
                        bag.pendingInvTTL = Math.max(prevTTL, BASE_TTL);
                        // Keep legacy short suppress counter as a fallback (not used if pendingInvTitles is active)
                        bag.suppressInventoryContextTurns = Math.max(0, Number(bag.suppressInventoryContextTurns || 0));
                        state._ACS = bag;
                    }
                } catch { /* noop */ }
                if (A.SHOW_TOASTS) {
                    const msg = created.length ? ("Created item cards: " + created.join(", ")) : "No new item cards needed.";
                    state.message = (state.message || "") + "\nACS: /inv — " + msg;
                }
    ...
    case "context": {
        ...
        try {
            ...
            if (Array.isArray(bag.pendingInvTitles) && bag.pendingInvTitles.length) {
                const remaining = [];
                for (const title of bag.pendingInvTitles) {
                    try {
                        const exists = !!API.getCard(c => (c && c.title === title));
                        if (!exists) remaining.push(title);
                    } catch {
                        // If API lookup failed, keep it pending to recheck next turn
                        remaining.push(title);
                    }
                }
                bag.pendingInvTitles = remaining;
                // Decrement TTL and decide suppression
                bag.pendingInvTTL = Math.max(0, Number(bag.pendingInvTTL || 0) - 1);
                ...
                bag.pendingInvTitles = [];
                bag.pendingInvTTL = 0;
                state._ACS = bag;
            }
            // Legacy fallback: short fixed suppression window if present
            if (bag.suppressInventoryContextTurns && 0 < bag.suppressInventoryContextTurns) {
                bag.suppressInventoryContextTurns = Math.max(0, Number(bag.suppressInventoryContextTurns) - 1);
                state._ACS = bag;
                return [TEXT, STOP];
            }
        } catch { /* noop */ }
    ...
    case "output": {
        ...
        try {
            ...
            if (deltas.length) {
                ...
                if (inv) {
                    const map = parseInventoryMap(inv.description);
                    applyInventoryDeltas(map, deltas);
                    inv.description = writeInventoryDescription(inv.description, map);
                    if (A.SHOW_TOASTS) {
...
// Expose ACS for optional local testing without affecting AID
try { globalThis._ACS_forTest = ACS; } catch {}