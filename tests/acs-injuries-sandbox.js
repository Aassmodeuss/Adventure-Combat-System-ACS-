...
(function main(){
  ...
  // Prefer two contiguous sentences for richer entries (injury)
  runOutput('{\\injury}A jagged blade slices across your palm. Blood beads quickly along the cut.{\\!injury}');
  let desc2 = getCardDesc('Player Injuries');
  assert(/- \[injury\] A jagged blade slices across your palm\. Blood beads quickly along the cut\./i.test(desc2), 'Two-sentence injury should be captured as a single entry');

  // Prefer two contiguous sentences for richer entries (heal)
  runOutput('{\\heal}You pour antiseptic over the wound. The sting fades and the bleeding slows.{\\!heal}');
  desc2 = getCardDesc('Player Injuries');
  assert(/- \[heal\] You pour antiseptic over the wound\. The sting fades and the bleeding slows\./i.test(desc2), 'Two-sentence heal should be captured as a single entry');
